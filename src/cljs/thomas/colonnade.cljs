...
(defn enum-constraint
  [{:keys [via] :as col} owner]
  (reify
    om/IRender
    (render [_]
      (let [schema (om/observe owner (schema))
        ...
        (dom/select
         {:value (str (:constrain col))
          ...
          (fn [e]
            (om/update!
             col
             :constrain (reader/read-string (.. e -target -value))))}
         (dom/option nil)
         (for [ev enum-values
               :let [id (:db/ident ev)]]
           ...
           (if (:required col)
             (if via-xref
               ...
               [[(symbol (str "?" (:from col)))
                 via
                 (symbol (str "?" k))]])
             ; vs not defined for xrefs
             (if (= (:db/cardinality vs) :db.cardinality/one)   
               [[(list 'get-else
                       '$
                       (symbol (str "?" (:from col)))
                       (symbol via)
             ...
             (let [rule (symbol (str "maybe-" k))
               ...
               [[(list rule ?a ?b)
                ...
                [(list rule ?a ?b)
                 ...
                 [(list
                   'ground
                   (if (= (:db/valueType vs) :db.type/ref)
                     -1
                     :none)) ?b]]])
             via-xref
             (if (= (count (str/split (namespace via-xref) #"\.")) 2)
  ...
  (reify
    ...
    (render [_]
      (dom/div
       ...
       (let [vizcols (filter
                      :visible
                      (for [[cid col] (or columns (:columns results))]
                        col))]
         (dom/table
          #js
           ...
           (let [export-keyset (#{"keyset" "ace" "acets"} export-format)
             ...
             (list
              ...
              (dom/div
               ...
               (dom/button
                {:on-click
                 #(run-query (max 0 (- offset page-size)) page-size)
                 :disabled back-disabled}
               ...
               (dom/button
                {:on-click
                 #(run-query
                   (* page-size (quot (dec (:count results)) page-size))
                   page-size)
                 :disabled fwd-disabled}
  ...
  (reify
    ...
    (will-mount [_]
      (edn-xhr "/schema"
        (fn [resp]
          (om/update!
           app {:schema (process-schema resp)
                :timeout 5000
                :col-seed 1
                :columns  (array-map
              ...
              #(om/transact!
                ...
                (fn [{:keys [col-seed] :as app}]
                  (assoc app
                         :columns (array-map
                                   "col1"
                                   (assoc column-template :root true))
                         :col-seed 1)))
              :style {:float "right"}}
             "Clear query")
